<?php
/**
 * List testimonials functionality of the plugin.
 *
...
$query        = "SELECT ID,post_title FROM  {$tbPost} WHERE post_type = 'superb_testimonials' AND post_status = 'publish' LIMIT {$start}, {$numOfRecords}";
        ...
        <div class="gp-no-records">
            <div class="gp-no-records-box">
                ...
                <div class="gp-no-records-bottom">
                    <a href="<?php echo esc_url(admin_url('admin.php?page=superb-testimonials&task=add-new')) ?>" class="add-new-form"><i class="fas fa-plus"></i> <?php esc_html_e(" Create your first Testimonial", "superb-testimonials"); ?></a>
                </div>
            </div>
        </div>
    <?php } else { ?>
        <div class="gp-form-header">
            <div class="gp-header-left">
                <?php esc_html_e("All Testimonials", "superb-testimonials"); ?>
            </div>
        </div>
        <div class="gp-dashboard-data">
            <div class="gp-header">
                <a href="<?php echo esc_url(admin_url('admin.php?page=superb-testimonials&task=add-new')) ?>" class="add-new-form"><i class="fas fa-plus"></i> <?php esc_html_e("Add New Testimonial", "superb-testimonials"); ?></a>
            </div>
                    ...
                    <div id="ajax-table-data">
                        ...
                        if (!empty($postData)) { ?>
                            <table class="dashboard-list-table">
                                <thead>
                                <tr>
                                    <th class="status-col"><?php esc_html_e("Image", "superb-testimonials"); ?></th>
                                ...
                                <?php foreach ($postData as $post) {
                                    $testimonialSetting = get_post_meta($post->ID, "testimonial_settings", true);
                                    ...
                                    <tr data-nonce="<?php echo wp_create_nonce("testimonial_action_".esc_attr($post->ID)) ?>" class="testimonial-col-<?php echo esc_attr($post->ID) ?>">
                                        <td class="status-col actions">
                                            <?php if (!empty($imageUrl)) { ?>
                                                <img src="<?php echo esc_url($imageUrl)  ?>" class="client-profile">
                                            <?php } ?>
                                        </td>
                                        <td><?php echo esc_attr($post->post_title) ?></td>
                                        <td class="action-col actions">
                                            <a data-ginger-tooltip="<?php esc_html_e("Edit testimonial", "superb-testimonials"); ?>"
                                               href="<?php echo esc_url(admin_url('admin.php?page=superb-testimonials&task=edit-testimonial&edit='.esc_attr($post->ID).'&nonce='.wp_create_nonce('edit_testimonials_'.esc_attr($post->ID)).'&paged='.esc_attr($paged))) ?>"
                                               class="edit-form"><i class="fas fa-edit"></i></a>
                                            <a data-ginger-tooltip="<?php esc_html_e("Remove", "superb-testimonials"); ?>" href="javascript:;"
                                               class="remove-testimonial remove-item" data-id="<?php echo esc_attr($post->ID) ?>"><i class="fas fa-trash-alt"></i></a>
                                        </td>
                                    </tr>
                                <?php }//end foreach
                                ?>
                        ...
                        <div class="ajax-pagination">
                           <?php
                            ...
                            $totalTestimonials = $wpdb->get_var("SELECT count(*) as count_num FROM  $tbPost WHERE post_type = 'superb_testimonials' AND post_status = 'publish'");
